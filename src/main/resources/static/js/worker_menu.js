...
async function load_content(value){
    if(value == "order"){
        await load_orderContent();
    }
    else {
        await load_serviceContent();
    }

}

async function load_orderContent(){
    const content = document.querySelector('.content');
...
async function get_orders(){
    const url = "http://localhost:8080/api/user/find/workerOrders";
    const body = {
        method : "GET",
        headers:{
...
async function redact_order(id,modal) {
    ...
    modal.querySelector(".venicle-modal-info").textContent = `${order.vehicle.make}, '${order.venicle.model}'   Госномер:${order.vehicle.number}`;
...
async function save_changes(order_id,modal) {
    ...
    console.log(body);
    const url = `http://localhost:8080/api/order/update_status/${order_id}`;
    const response = await do_request(url,body);
    if(response.ok){
        await load_orderContent();
...
async function load_serviceContent(){
    const content = document.querySelector('.content');
